...
h.type="text/css";h.charset="utf-8";h.rel="stylesheet";a=0;for(e=c.length;a<e;a++){i=[c[a][0],"{"];d=1;for(g=c[a].length;d<g;d++)i.push(c[a][d].join(":"),";");i.push("}");c[a]=i.join("")}c=c.join("");a=document.getElementsByTagName("link");if(a.length==0)a=document.getElementsByTagName("style");a.length!=0?document.getElementsByTagName("head")[0].insertBefore(h,a[0]):document.getElementsByTagName("head")[0].appendChild(h);if(h.styleSheet)h.styleSheet.cssText=c;else b(h).text(c)}var w,k={},f,s,p,q,
r,o,l,n,j;b.fn.hslLiteColorPicker=function(a){var e,d=[];if(typeof a=="string"&&this.data("hslLiteColorPickerId"))return a==="color"?b.colors(k[this.data("hslLiteColorPickerId")].hsl,"array1Circle2Percentage","HSL"):k[this.data("hslLiteColorPickerId")][a];w||z();e=this.each(function(){t(b(this));if(typeof a=="object"){var g=b.colors.models.HSL.parameterIndexes,c;for(c in a){if(c in g){k[j].hsl[g[c]]=a[c];delete a[c]}if(c==="color"){if(a[c]instanceof b.colors===false)a[c]=b.colors(a[c]);k[j].hsl=a[c].model("HSL").format("array1Circle2Percentage");
delete a[c]}}b.extend(k[j],a);k[j].hsl=b.colors.models.HSL.sanitize(k[j].hsl).slice(0,3);t(o);if("disabled"in a)a.disabled?l.addClass("disabled"):l.removeClass("disabled")}else a==="hsl"&&d.push(f);v()});return d.length>1?d:d.length==1?d[0]:e};if(b.colors.pickers===undefined)b.colors.pickers={};var m=b("script[src]");if(m.length>0){m=m.eq(m.length-1).attr("src").split("/");m.pop();m=m.join("/")+"/"}else m="";b.colors.pickers.hslLite={pathToImages:m}})(jQuery);
