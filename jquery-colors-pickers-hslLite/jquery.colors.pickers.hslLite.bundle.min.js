...
},toString:function(a){if(!a||a in e.formats===false)a=e.defaultString;try{return this.format(a).toString()}catch(b){return this.format(e.defaultString).toString()}},isFormat:function(a){if(a&&a in e.formats)return e.formats[a].validate(this.inputColor);else throw"Format does not exist";},mixWith:function(a,b){var c=0,f,h,i,m;if(typeof b=="number")m=b;f=this.get();i=this.model();if(d.isArray(a))try{h=e(a)}catch(o){for(c=0;c<a.length;c++){if(a[c]instanceof e==false)a[c]=e(a[c]);h=c==0?a[c].model(i):
h.mixWith(a[c],c/(c+1))}}else{if(a instanceof e==false)a=e(a);h=a}h=h.model(i).get();f=f.slice(0);h=h.slice(0);if(m==undefined)m=c?1/(c+1):0.5;for(c=0;c<f.length;c++){if("cycleMixes"in e.models[i]&&e.models[i].cycleMixes[c])if(e.models[i].cycleMixes[c]>0)for(;f[c]>h[c];)h[c]+=e.models[i].cycleMixes[c];else for(;f[c]<h[c];)h[c]+=e.models[i].cycleMixes[c];f[c]=f[c]*m+h[c]*(1-m)}f=e.models[i].sanitize(f);return d.extend(d.colors(),{color:f,currentModel:i})}};e.formats={array3Octet:{validate:function(a,
...
3)}}},array1Circle2Percentage1Normalized:{validate:function(a){return y.array1Circle2Percentage.validate(a,4)},toModel:{HSL:function(a){return a.slice(0,4)}},fromModel:{HSL:function(a){return a.slice(0,4)}}}};d.extend(true,d.colors.convertModels,{RGB:{HSL:function(a){var b=a[0]/255,c=a[1]/255,f=a[2]/255,h=Math.max(b,c,f),i=Math.min(b,c,f),m=h-i,o=(h+i)/2;if(h==i)b=i=0;else{i=o>0.5?m/(2-h-i):m/(h+i);b=h==b?(c-f)/m+(c<f?6:0):h==c?(f-b)/m+2:(b-c)/m+4;b/=6}return[b*360,i*100,o*100,a[3]]}},HSL:{RGB:function(a){function b(o,
q,n){if(n<0)n+=1;if(n>1)n-=1;return n<1/6?o+(q-o)*6*n:n<0.5?q:n<2/3?o+(q-o)*(2/3-n)*6:o}var c,f,h,i,m;h=a[0]/360;f=a[1]/100;c=a[2]/100;if(f==0)c=f=h=c;else{i=c<0.5?c*(1+f):c+f-c*f;m=2*c-i;c=b(m,i,h+1/3);f=b(m,i,h);h=b(m,i,h-1/3)}return[c*255,f*255,h*255,a[3]]}}});d.extend(d.colors.models,v);d.extend(d.colors.formats,y)})(jQuery);
(function(d){function u(){var g=d("<div/>").css({width:198,backgroundRepeat:"repeat-y",position:"relative",overflow:"hidden"}).addClass("scale"),l=d("<div/>").css({zIndex:1,bottom:0,backgroundRepeat:"repeat-y"}),k=d("<span/>").css({position:"absolute",zIndex:3}),p=d("<span/>").css({bottom:0}).addClass("handle");l.add(p).css({position:"absolute",top:0,left:0,right:0});q=d("<div/>").addClass("hslLiteColorPicker");q.append(g.clone().addClass("h sprite").data("arrayIndex",0).append(l.clone())).append(g.clone().addClass("s").data("arrayIndex",
1).append(l.clone().addClass("sprite").css({backgroundPosition:"-792px 0"})).append(l.clone().addClass("sprite").css({backgroundPosition:"-396px 0"})).append(l.clone().addClass("sprite").css({backgroundPosition:"-594px 0"}))).append(g.clone().addClass("l sprite").data("arrayIndex",2).css({backgroundPosition:"-198px 0"}));o.append(q);n=q.children("div");p.append(k.clone().text("\u25bc")).append(k.clone().text("\u25b2"));k.addClass("outline").css({zIndex:2});p.append(k.clone().text("\u25bc")).append(k.clone().text("\u25b2"));
n.append(p).children().children().each(function(j){var r=d(this),s="top";if(j%2!=0)s="bottom";r.css({left:-r.width()/2-1}).css(s,-r.height()/2)});c=[Math.random()*360,Math.random()*100,Math.random()*100];f=false;h=i=m=null;n.mousedown(x)}function x(g){function l(s){var z=100;s=(s.pageX-j)/200;s=s>1?1:s<0?0:s;if(r==0)z=360;c[r]=s*z;e();d.isFunction(i)&&i(d.colors(c,"array1Circle2Percentage","HSL"));return false}function k(){d.isFunction(m)&&m(d.colors(c,"array1Circle2Percentage","HSL"));d(document).unbind("mousemove",
l).unbind("mouseup",k);q.data("dragging",false)}var p=d(g.currentTarget),j=p.offset().left,r=p.data("arrayIndex");v(d(this).closest(".hslLiteColorPicker").parent());if(f)return true;d.isFunction(h)&&h(d.colors(c,"array1Circle2Percentage","HSL"));if(!q.data("dragging")){d(document).bind("mousemove",l).bind("mouseup",k);q.data("dragging",true)}l(g);return false}function e(){var g,l;n.each(function(){var k=d(this),p=k.data("arrayIndex"),j=100;if(p==0)j=360;k.children("span").css({left:c[p]*200/j})});
if(c[2]>50){l=(c[2]-50)*2/100;g=[l,1-l,0]}else{l=(50-c[2])*2/100;g=[0,1-l,l]}n.filter(".h").children("div").css({backgroundColor:d.colors([0,0,c[2]],"array1Circle2Percentage","HSL").toString("rgb")}).css("opacity",Math.max(l,(100-c[1])/100));n.filter(".s").css({backgroundColor:d.colors([c[0],100,50],"array1Circle2Percentage","HSL").toString("rgb")}).children("div").each(function(k){d(this).css("opacity",g[k])});n.filter(".l").css({backgroundColor:d.colors([c[0],c[1],50],"array1Circle2Percentage",
"HSL").toString("rgb")})}function v(g){o=g;if(o.data("hslLiteColorPickerId")){q=o.children();n=q.children("div.scale");t=o.data("hslLiteColorPickerId");c=b[t].hsl;f=b[t].disabled;h=b[t].onStartChange;i=b[t].onChange;m=b[t].onEndChange}else{t=(new Date).getTime();o.data("hslLiteColorPickerId",t);u();b[t]={hsl:c,disabled:f,onStartChange:h,onChange:i,onEndChange:m}}}function y(){var g,l,k,p,j=[[".hslLiteColorPicker",["display","inline-block"]],[".hslLiteColorPicker>div.scale",["height","1.5em"],["border",
