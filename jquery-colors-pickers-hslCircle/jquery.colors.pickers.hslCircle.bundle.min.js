...
a;};e.fn=e.prototype={get:A,set:A,model:function(a){if(a===undefined)return this.currentModel;else if(a==this.currentModel)return this;else if(a in e.models){this.color=w(e.convertModels[this.currentModel],a,this.color);this.currentModel=a;return this}else throw"Model does not exist";},format:function(a){var b=this.currentModel=="RGB"&&a!=="array3Octet1Normalized"?this.format("array3Octet1Normalized"):this.color;if(a&&a in e.formats)return w(e.formats[a].fromModel,this.currentModel,b);else throw"Format does not exist";
...
a.push(0);a=a.slice(0,4);for(b=0;b<a.length;b++){a[b]||(a[b]=0);if(b<3){if(a[b]>255)a[b]=255;if(a[b]<0)a[b]=0}else if(b==3){a[b]=parseFloat(a[b]);if(a[b]>1)a[b]=1;if(a[b]<0)a[b]=0}}return a},parameterIndexes:{r:0,g:1,b:2,a:3,red:0,green:1,blue:2,alpha:3}}};e.convertModels={};e.defaultInputModel=e.defaultModel="RGB";e.defaultString="rgb";d.colors===undefined&&d.extend({colors:e});var y={HSL:{sanitize:function(a){var b;if(!a||!d.isArray(a))a=[Math.floor(361*Math.random()),Math.floor(101*Math.random()),
...
if(h==0)c=h=n=c;else{o=c<0.5?c*(1+h):c+h-c*h;r=2*c-o;c=b(r,o,n+1/3);h=b(r,o,n);n=b(r,o,n-1/3)}return[c*255,h*255,n*255,a[3]]}}});d.extend(d.colors.models,y);d.extend(d.colors.formats,B)})(jQuery);
(function(d){function w(){var f=d("<div/>").addClass("scale"),j=d("<div/>").css({zIndex:1,width:160,height:160}),l=d("<span/>").css({position:"absolute",zIndex:3}),m=l.clone().addClass("outline").css({zIndex:2}),g=d("<span/>").addClass("handle");j.add(g).css({position:"absolute",top:0,left:0});q=d("<div/>").addClass("hslCircleColorPicker").css({position:"relative",width:160,height:185});q.append(f.clone().addClass("h").data("arrayIndex",0).addClass("hSprite").css({width:160,height:160}).append(j.clone().addClass("hSprite").css({backgroundPosition:"-320px 0"})).append(j.clone().addClass("hSprite").css({backgroundPosition:"-160px 0"})));
...
c[1],50],"array1Circle2Percentage","HSL").toString("rgb")})}function y(f){s=f;if(s.data("hslCircleColorPickerId")){q=s.children();p=q.children("div.scale");t=s.data("hslCircleColorPickerId");c=b[t].hsl;h=b[t].disabled;n=b[t].onStartChange;o=b[t].onChange;r=b[t].onEndChange}else{t=(new Date).getTime();s.data("hslCircleColorPickerId",t);w();b[t]={hsl:c,disabled:h,onStartChange:n,onChange:o,onEndChange:r}}}function B(){function f(j){var l,m,g,v,i=[[".hslCircleColorPicker",["display","inline-block"]],
...
"18px"]],[".hslCircleColorPicker.disabled>div.scale",["cursor","default"]],[".hslCircleColorPicker.disabled>div.scale>span.handle>span",["opacity","0.5"],["filter","alpha(opacity=50)"]]],k,u,x=document.getElementsByTagName("link");if(x.length==0)x=document.getElementsByTagName("style");if(j===true||this.width!=1||this.height!=1)i=i.concat([[".hslCircleColorPicker .hSprite",["background-image","url('"+d.colors.pickers.hslCircle.pathToImages+"jquery.colors.pickers.hslCircle.sprite.h.png')"]],[".hslCircleColorPicker .sSprite",
["background-image","url('"+d.colors.pickers.hslCircle.pathToImages+"jquery.colors.pickers.hslCircle.sprite.s.png')"]],[".hslCircleColorPicker>div.scale.l",["background-image","url('"+d.colors.pickers.hslCircle.pathToImages+"jquery.colors.pickers.hslCircle.l.png')"]]]);else{k=document.createElement("link");k.type="text/css";k.charset="utf-8";k.rel="stylesheet";k.href=d.colors.pickers.hslCircle.pathToImages+"jquery.colors.pickers.hslCircle.css";x.length!=0?document.getElementsByTagName("head")[0].insertBefore(k,
x[0]):document.getElementsByTagName("head")[0].appendChild(k)}k=document.createElement("style");k.type="text/css";k.charset="utf-8";k.rel="stylesheet";l=0;for(m=i.length;l<m;l++){u=[i[l][0],"{"];g=1;for(v=i[l].length;g<v;g++)u.push(i[l][g].join(":"),";");u.push("}");i[l]=u.join("")}i=i.join("");x.length!=0?document.getElementsByTagName("head")[0].insertBefore(k,x[0]):document.getElementsByTagName("head")[0].appendChild(k);if(k.styleSheet)k.styleSheet.cssText=i;else d(k).text(i);j!==true&&d(this).remove()}
a=true;/MSIE/.test(navigator.appVersion)?f(true):d("<img/>").attr("src","data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==").one("error load",f).appendTo("body")}var a,b={},c,h,n,o,r,s,q,p,t;d.fn.hslCircleColorPicker=function(f){var j,l=[];if(typeof f=="string"&&this.data("hslCircleColorPickerId"))return f==="color"?d.colors(b[this.data("hslCircleColorPickerId")].hsl,"array1Circle2Percentage","HSL"):b[this.data("hslCircleColorPickerId")][f];a||B();j=this.each(function(){y(d(this));
if(typeof f=="object"){var m=d.colors.models.HSL.parameterIndexes,g;for(g in f){if(g in m){b[t].hsl[m[g]]=f[g];delete f[g]}if(g==="color"){if(f[g]instanceof d.colors===false)f[g]=d.colors(f[g]);b[t].hsl=f[g].model("HSL").format("array1Circle2Percentage");delete f[g]}}d.extend(b[t],f);b[t].hsl=d.colors.models.HSL.sanitize(b[t].hsl).slice(0,3);y(s);if("disabled"in f)f.disabled?q.addClass("disabled"):q.removeClass("disabled")}else f==="hsl"&&l.push(c);e()});return l.length>1?l:l.length==1?l[0]:j};if(d.colors.pickers===
