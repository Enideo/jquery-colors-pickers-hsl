...
100];v=false;q=r=s=null;o.mousedown(A)}function A(b){function e(d){var f=100;if(n==0){d=Math.atan2(d.pageX-c.left,c.top-d.pageY)/6.28;if(d<0)d+=1;f=360}else d=n==1?(100-(d.pageY-c.top))/100:(d.pageX-c.left)/160;d=d>1?1:d<0?0:d;i[n]=d*f;x();a.isFunction(r)&&r(a.colors(i,"array1Circle2Percentage","HSL"));return false}function g(){a.isFunction(s)&&s(a.colors(i,"array1Circle2Percentage","HSL"));a(document).unbind("mousemove",e).unbind("mouseup",g);k.data("dragging",false)}var h=a(b.currentTarget),c=h.offset(),
n=h.data("arrayIndex");if(n==0){c.top+=80;c.left+=80}w(a(this).closest(".hslCircleColorPicker").parent());if(v)return true;a.isFunction(q)&&q(a.colors(i,"array1Circle2Percentage","HSL"));if(!k.data("dragging")){a(document).bind("mousemove",e).bind("mouseup",g);k.data("dragging",true)}e(b);return false}function x(){var b,e;o.each(function(){var h=a(this),c=h.data("arrayIndex");h=h.children("span");if(c==0){c=i[c]/360*6.28;h.css({top:-Math.cos(c)*70+80,left:Math.sin(c)*70+80})}else c==1?h.css({top:98-
...
i[1],50],"array1Circle2Percentage","HSL").toString("rgb")})}function w(b){p=b;if(p.data("hslCircleColorPickerId")){k=p.children();o=k.children("div.scale");j=p.data("hslCircleColorPickerId");i=l[j].hsl;v=l[j].disabled;q=l[j].onStartChange;r=l[j].onChange;s=l[j].onEndChange}else{j=(new Date).getTime();p.data("hslCircleColorPickerId",j);z();l[j]={hsl:i,disabled:v,onStartChange:q,onChange:r,onEndChange:s}}}function B(){function b(e){var g,h,c,n,d=[[".hslCircleColorPicker",["display","inline-block"]],
...
"18px"]],[".hslCircleColorPicker.disabled>div.scale",["cursor","default"]],[".hslCircleColorPicker.disabled>div.scale>span.handle>span",["opacity","0.5"],["filter","alpha(opacity=50)"]]],f,m,t=document.getElementsByTagName("link");if(t.length==0)t=document.getElementsByTagName("style");if(e===true||this.width!=1||this.height!=1)d=d.concat([[".hslCircleColorPicker .hSprite",["background-image","url('"+a.colors.pickers.hslCircle.pathToImages+"jquery.colors.pickers.hslCircle.sprite.h.png')"]],[".hslCircleColorPicker .sSprite",
["background-image","url('"+a.colors.pickers.hslCircle.pathToImages+"jquery.colors.pickers.hslCircle.sprite.s.png')"]],[".hslCircleColorPicker>div.scale.l",["background-image","url('"+a.colors.pickers.hslCircle.pathToImages+"jquery.colors.pickers.hslCircle.l.png')"]]]);else{f=document.createElement("link");f.type="text/css";f.charset="utf-8";f.rel="stylesheet";f.href=a.colors.pickers.hslCircle.pathToImages+"jquery.colors.pickers.hslCircle.css";t.length!=0?document.getElementsByTagName("head")[0].insertBefore(f,
t[0]):document.getElementsByTagName("head")[0].appendChild(f)}f=document.createElement("style");f.type="text/css";f.charset="utf-8";f.rel="stylesheet";g=0;for(h=d.length;g<h;g++){m=[d[g][0],"{"];c=1;for(n=d[g].length;c<n;c++)m.push(d[g][c].join(":"),";");m.push("}");d[g]=m.join("")}d=d.join("");t.length!=0?document.getElementsByTagName("head")[0].insertBefore(f,t[0]):document.getElementsByTagName("head")[0].appendChild(f);if(f.styleSheet)f.styleSheet.cssText=d;else a(f).text(d);e!==true&&a(this).remove()}
y=true;/MSIE/.test(navigator.appVersion)?b(true):a("<img/>").attr("src","data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==").one("error load",b).appendTo("body")}var y,l={},i,v,q,r,s,p,k,o,j;a.fn.hslCircleColorPicker=function(b){var e,g=[];if(typeof b=="string"&&this.data("hslCircleColorPickerId"))return b==="color"?a.colors(l[this.data("hslCircleColorPickerId")].hsl,"array1Circle2Percentage","HSL"):l[this.data("hslCircleColorPickerId")][b];y||B();e=this.each(function(){w(a(this));
if(typeof b=="object"){var h=a.colors.models.HSL.parameterIndexes,c;for(c in b){if(c in h){l[j].hsl[h[c]]=b[c];delete b[c]}if(c==="color"){if(b[c]instanceof a.colors===false)b[c]=a.colors(b[c]);l[j].hsl=b[c].model("HSL").format("array1Circle2Percentage");delete b[c]}}a.extend(l[j],b);l[j].hsl=a.colors.models.HSL.sanitize(l[j].hsl).slice(0,3);w(p);if("disabled"in b)b.disabled?k.addClass("disabled"):k.removeClass("disabled");a.isFunction(q)&&q(a.colors(i,"array1Circle2Percentage","HSL"));a.isFunction(r)&&
r(a.colors(i,"array1Circle2Percentage","HSL"));a.isFunction(s)&&s(a.colors(i,"array1Circle2Percentage","HSL"))}else b==="hsl"&&g.push(i);x()});return g.length>1?g:g.length==1?g[0]:e};if(a.colors.pickers===undefined)a.colors.pickers={};var u=a("script");u=u.eq(u.length-1).attr("src").split("/");u.pop();a.colors.pickers.hslCircle={pathToImages:u.join("/")+"/"}})(jQuery);
